...
app.post('/client_out', (req, res) => {
    

   console.log("-----------Client ID: ",req.body.id);

    for(i in clients_sockets){
        if(clients_sockets[i].id==req.body.id){
            //console.log(providers_sockets[i].id);
            
            io.sockets.sockets[(clients_sockets[i].socket)].disconnect();
            clients_sockets.splice(i, 1);
            
        }
    }
    console.log("--------Client Out--------");
    console.log(clients_sockets);
...
// var now = new Date();
// var millisTill10 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), 59);

// const date1 = new Date();
// const date2 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), 56);
...
io.on('connection', function (socket) {    ////// connection add eventlistner 
    ...
    try{
        ...
        if (UserType == 'Clients') {
            ...
            for(i in clients_sockets){
                if(clients_sockets[i].id==UserId){
                    //console.log(providers_sockets[i].id);
                    //providers_sockets.splice(i, 1);
                    //founded=1;
                    console.log("-----------Haredy-Client---------");
                    io.sockets.sockets[(clients_sockets[i].socket)].emit("wrong_mac");
                    io.sockets.sockets[(clients_sockets[i].socket)].disconnect();
                }
            }
            ...
            //console.log("Providers Old Sockets --> ",providers_sockets);

            for(i in providers_sockets){

                if(providers_sockets[i].id==UserId){
                    console.log("Founded ..");
                    io.sockets.sockets[(providers_sockets[i].socket)].emit("wrong_mac",{ mac: UserMac });
                    io.sockets.sockets[(providers_sockets[i].socket)].disconnect();
                }
            }
            ...
            //console.log("Providers new Sockerts", providers_sockets);
            console.log("------------------");
    ...
    // var now=new Date();
    // var d=new Date('7-13-2010 11:10:00');
    // var d2=new Date('7-13-2010 11:11:02');
    // var diffTime = Math.abs(d2 - d);
    // var nowPlus5 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), now.getMinutes() -20);
    // // setTimeout(function () {
    // //     console.log("Aloo");
    // //   }, diffTime);
    // setTimeout(function () {
    //     console.log(nowPlus5.toString());
    // }, 2000);

    socket.on('scheduled_now', function(data){
        ...
        try{
            ...
            if(data.confirm_schedule == 1){
                ...
                async function asyncGettingScheduledData(){

                    function gettingRequestData(){
                        return new Promise(function(resolve, reject){
                            connection.query(' SELECT service_type_id ,booking_id ,s_latitude ,s_longitude ,d_latitude ,d_longitude ,s_address_en ,s_address_ar ,d_address_en ,d_address_ar ,schedule_at ,id ,provider_id FROM user_requests WHERE status = "SCHEDULED" AND user_id = ' + "'" + user_id + "'"  ,function (error, results, fields){
                            
                                if(!error && results[0] != null){
                                    //console.log(results[0]);
                                    resolve([results[0].service_type_id,results[0].booking_id,results[0].s_latitude,results[0].s_longitude,results[0].d_latitude,results[0].d_longitude,results[0].s_address_en,results[0].s_address_ar,results[0].d_address_en,results[0].d_address_ar,results[0].schedule_at,results[0].id,results[0].provider_id]);
                                }
                                else{
                                    console.log("Error in gettingRequestData");
                                    //throw Error();
                                }
                            })
                        })
                    }
                    arr4=await gettingRequestData();
                    service_type= arr4[0];
                    bookingId= arr4[1];
                    s_latitude= arr4[2];
                    s_longitude= arr4[3];
                    ...
                    function gettingUserData(){
                        return new Promise(function(resolve, reject){
                            connection.query(' SELECT first_name,last_name,email,picture,rating,mobile FROM users WHERE id = ' + "'" + user_id + "'" ,function (error, results, fields){
                                if(!error && results[0] != null){
                                    resolve([results[0].first_name,results[0].last_name,results[0].email,results[0].picture,results[0].rating,results[0].mobile]);
                                }
                                else{
                                    console.log("Error In gettingUserData");
                                }
                            })
                        })
                    }
                    arr2=await gettingUserData();
                    first_name= arr2[0];
                    last_name=arr2[1];
                    email=arr2[2];
                    picture1=arr2[3];
                    ...
                    function gettingProviderData(){
                        return new Promise(function(resolve, reject){
                            connection.query(' SELECT email,mobile FROM providers WHERE id = ' + "'" + provider_id + "'" ,function (error, results, fields){
                                if(!error && results[0] != null){
                                    resolve([results[0].email,results[0].mobile]);
                                }
                                else{
                                    console.log("Error In gettingProviderData");
                                }
                            })
                        })
                    }
                    arr3=await gettingProviderData();
                    provider_email= arr3[0];
                    provider_phone= arr3[1];
                    //console.log({arr3});
                    ...
                    //var date= schedule_at;
                    console.log({schedule_at});

                    var now=new Date();
                    console.log({now});
                    var scheduledDate=new Date(schedule_at);
                    var diffTime = Math.abs(scheduledDate - now);
                    console.log({diffTime});

                    // eval('var ' + 'provider_setInterval' + provider_id + '= ' + 

                    //     setInterval(() => {
                    //         console.log("BsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBsBs");
                    //         Providerss[provider_id].emit('request_range', responseObj);
                    //         console.log("TimeOut Emit Done ..");
                    //         //settripid()
                    //         responseObj.request.rejectedProviders = [];
                    //         responseObj.request.request_id = Trip_id;
                    //         responseObj.request.Trip_id = Trip_id;
                    //         responseObj.request.scheduled = 1;
                    //         responseObj.request.scheduled_at = scheduledDate;
                    //         Requests['Request' + responseObj.UserId] = responseObj;
                    //     }, 1000)

                    // + ';');

                    // setTimeout(() => {
                    //     clearInterval(  eval('provider_setInterval' + provider_id ) );
                    // }, 10000); 
                    ...
                    Clientss[user_id].emit('schedule_response', obj);
                ...
                asyncGettingScheduledData();
                
                
                //////////////////////////////

                //emitOn(Providerss[212], 'confirm', { case: "true", message: "hello fred", Trip_id: 75, provider_email:"aa@aa.com", provider_phone: 111 }, data.id);
                //console.log({Providerss});


                //Providerss[215].emit('request_range', "Alo");
            }
            else if(data.confirm_schedule == 0){
                //console.log({Trip_id});
                ...
                connection.query(' UPDATE user_requests SET   status =  ' +
                    ...
                    + "  NOW()," + "cancelled_by ="+ '"USER"' +"WHERE status = 'SCHEDULED' " +
                    'AND user_id = ' +
                    "'" + user_id + "'" +
                    '   ', (error, results, fields) => {
                        if (!error) {
                            ...
                            //return results
                            console.log("ScheduleEvent: mysqlCancelled");
            ...
            console.log("----------End Of Schedule Event---------------");

        }catch(err1){
            err1.where="Event: scheduled_now";
            err1.date=new Date().toLocaleString();
            err1.body=err1.stack;
            err1.data=data;
    ...
    socket.on('request_range_provider', function (data) {
        try{
            ...
            if (data.Status == 'Reject') {
                ...
                //console.log("Request With Rejected: ",Requests['Request' + data.request.id].request.rejectedProviders);
                ...
                if( Requests['Request' + data.request.id].request.scheduled == 0 ){
                    Providerss[data.response.id].emit('not_confirm', { Trip_id: data.request.Trip_id });
                    DeleteFromRequestedProvider(data.response.id);

                    
                    Requests['Request' + data.request.id].request.rejectedProviders.push(parseInt(data.response.id));
                    console.log("Request With Rejected: ",Requests['Request' + data.request.id]);
                    

                    console.log("------------------------------------");
                }else if(
                    Requests['Request' + data.request.id].request.scheduled == 1 ){
                    Providerss[data.response.id].emit('not_confirm', { Trip_id: data.request.Trip_id });
                    DeleteFromRequestedProvider(data.response.id);

                    // var scheduledDate=data.UserID;
                    
                    // clearInterval(  eval('provider_setInterval' + data.response.id ) );
                    // console.log("----------- Interval Cleared ---------------");

                    Requests['Request' + data.request.id].request.rejectedProviders.push(parseInt(data.response.id));
                    console.log("Request With Rejected: ",Requests['Request' + data.request.id]);

                    var the_deleted_request=Requests['Request' + data.request.id];
                    delete Requests['Request' + data.request.id];

                    var now=new Date();
                    console.log({now});
                    //var scheduledDate=new Date(data.request.scheduled_at);
                    var scheduledDate= the_deleted_request.request.scheduled_at;
                    var time_before_15= new Date(scheduledDate.getFullYear(), scheduledDate.getMonth(), scheduledDate.getDate(), scheduledDate.getHours(), scheduledDate.getMinutes() -15 );

                    console.log({scheduledDate});
                    console.log({time_before_15});

                    var diffTime = Math.abs(time_before_15 - now);
                    console.log({diffTime});

                    setTimeout(function () {
                        
                        //settripid()
                        //the_deleted_request.request.rejectedProviders = [];
                        the_deleted_request.request.request_id = data.request.Trip_id;
                        the_deleted_request.request.Trip_id = data.request.Trip_id;
                        the_deleted_request.request.scheduled = 0;
                        Requests['Request' + the_deleted_request.UserId] = the_deleted_request;
                        SendRequest(Requests['Request' + the_deleted_request.UserId]);

                        console.log(".. Timeout In Reject ..");
                    }, diffTime);
                ...
                // if(Requests['Request' + data.request.id].request.schedule_at == 1){
                //     clearInterval(  eval('provider_setInterval' + data.response.id ) );
                //     console.log("----------- Interval Cleared ---------------");
                // }
                ...
                console.log("Array OF Request ------",Requests);
        ...
        try{
            ...
            if (!error) {
                //if(results[0] !=null){
                    ...
                    var obj = {
                        "providerFirstName": pickkkk.first_name,
                        "providerLastName": pickkkk.last_name,
                        "providerEmail": pickkkk.email,
                        "providerRating": pickkkk.rating,
                        "providerMobile": pickkkk.mobile,
                        "providerAvatar": pickkkk.picture
                    }
            ...
            connection.connect();
            connection.query(' SELECT id FROM user_requests WHERE status = "SCHEDULED" AND user_id = ' + "'" + user_id + "'"  ,function (error, results, fields){
                            
                if(!error && results[0] != null){
                    //console.log(results[0]);
                    Trip_id = results[0].id;
                    console.log("From SQL: ", results[0].id);
                }
                else{
                    console.log("Error in gettingTripID");
                    //throw Error();
                }
                    ...
                    function (error, response, body) {
                        ...
                        if (!error && response.statusCode == 200) {
                            ...
                            if (typeof response.body != undefined) {
                                ...
                                request.post(
                                    'http://5.189.186.251/ailbaz_server/public/api/provider/trip_payment',
                                    {
                                        json: {
                                            "Trip_id": data.Trip_id,
                                            "paid": data.paid
                                    ...
                                    function (error, response, body) {
                                        if (!error && response.statusCode == 200) {
                                            console.log("-----------2nd Bill-------------");
                                        }
                                        else{
                                            console.log("-----------Error In 2nd Bill-------------");
                                        }
                                    }
        ...
        try{    
            ...
            if(datas.sub == 0){
                ...
                if ((datas.provider_id == '0' || datas.provider_id == '') && (datas.Trip_id == '0'|| datas.Trip_id == '')) {
                    ...
                    console.log("Requests Array After Provider Accept ..",Requests);
                    if (datas.client_id != '' && datas.client_id != '0') {
                        console.log("start_not_confirm start_not_confirm start_not_confirm start_not_confirm");
