                ...
                // connection.query('select * from cars WHERE id =56 ', (error, results2 ,fields) => {
                    connection.query('select * from cars ' +
                        ...
                        '', (error, results, fields) => {
                            ...
                            if (!error) {
                                console.log("data has been updated after driver data provider---------------------------------", results[0]);
                            //  return results2;
                            } else {
                                console.log(error)
                                return error;
            ...
            function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    if (typeof response.body != undefined) {
                        ...
                        var bill = {
                            fixed_price: Math.round(response.body.data.fixed * 10) / 10,//السعر الثابت
                            distance: Math.round(response.body.data.distance * 10) / 10,//المسافة
                            distance_price: Math.round(response.body.data.price * response.body.data.distance * 10) / 10,//تكلفة المسافة
                            wattingTime: Math.round(response.body.data.WaitingTime),//وقت الانتظار
                            time_price: Math.round(response.body.data.min_wait_price * 10) / 10,//تكلفة الانتظار
                            tripTime: Math.round(response.body.data.time_trip),//وقت الرحلة
                            watting_price: Math.round(response.body.data.time_trip_price *10) / 10,//تكلفة وقت الرحلة

                            tax: Math.round(response.body.data.tax * 10) / 10,//الضريبة
                            total_price: Math.round(response.body.data.total * 10) / 10,//الاجمالي 

                           // watting_price: Math.round(response.body.data.WaitingPrice * 10) / 10,

                            discount_wallet: Math.round(response.body.data.discount_wallet * 10) / 10,//تخفيض لا يعرض
                            Trip_id: dropedddd.Trip_id
                        };
                        var info = {};
                        info.bill = bill;
            ...
            '', (error, results, fields) => {
                ...
                if (!error) {
                    console.log("get data from data base this data is ",results[0].status );
                            if(results[0].status == 'ARRIVED' ||  results[0].status == 'ACCEPTED'){
